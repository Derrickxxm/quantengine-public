...
def test_gate_command_reuses_demo_artifacts(tmp_path):
    ...
    assert gate["release_gate"] == "pass"
    assert gate["checks"]["artifact_integrity"] == "pass"


def test_gate_command_fails_tampered_artifact(tmp_path):
    assert main(["demo", "--artifact-dir", str(tmp_path)]) == 0
    (tmp_path / "actual_state.json").write_text('{"tampered": true}\n')
    ...
    assert gate["checks"]["artifact_integrity"] == "fail"
...
def test_validate_command_fails_tampered_artifact(tmp_path):
    assert main(["demo", "--artifact-dir", str(tmp_path)]) == 0
    (tmp_path / "actual_state.json").write_text('{"tampered": true}\n')
    ...
    assert gate["release_gate"] == "fail"
    assert gate["checks"]["artifact_integrity"] == "fail"
    assert gate["artifact_mismatches"][0]["error"] == "sha256 mismatch"
