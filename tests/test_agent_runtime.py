...
def test_run_state_serializes_and_resumes_after_tool_approval() -> None:
    ...
    state_json = runtime.serialize_state(paused.to_state())
    assert state_json["no_active_agent_run"] is True
    restored = asyncio.run(runtime.deserialize_state(agent, state_json))
