...
The public identity is an evidence-controlled AI software delivery system.
QuantEngine is the first reference implementation, not the definition of the
whole architecture.
...
| One-Agent-first rule | Why does a responsibility need a separate Agent? | explicit split criteria based on ownership, tools, permissions, context, or independent evaluation |
| Collaboration semantics | Is the specialist a tool, new owner, or independent reviewer? | Agent-as-tool, handoff, and independent-review contracts |
...
| Learning flywheel | How do failures and decisions improve the next cycle? | failure-to-eval-to-repair-to-regression-to-promotion path |
...
| Dynamic context assembly | How does each run receive current facts without trusting a static knowledge dump? | task/source/graph/evidence-bound context recipe and stale-context blocker |
| Task state and recovery | Which transitions, retries, resumptions, and invalidations are allowed? | finite state machine, next owner, idempotency, source-drift rules |
| Trace / evidence / eval / authority separation | What happened, what proves it, was it good, and what is allowed next? | distinct contracts and correlation identities |
...
| Independent release control | How is authority prevented from returning to the runtime producer? | zero-authority runtime evidence, runtime-bound Quality verdict, deterministic Release Controller |
